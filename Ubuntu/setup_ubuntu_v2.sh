#!/usr/bin/env bash
#
# setup_ubuntu.sh — Ubuntu workstation / development-server bootstrap.
#
# Intended targets: Ubuntu 22.04 and 24.04 with systemd.
...
#   INSTALL_POSTGRES=false INSTALL_SNAPS=false ./setup_ubuntu.sh
#
set -Eeuo pipefail
umask 022
...
INSTALL_FASTFETCH="${INSTALL_FASTFETCH:-false}"

# Workstation-specific: leave disabled on EC2 / remote servers.
...
# Preserve refs from your existing server_setup.sh.
# For stricter reproducibility, replace these with verified full commit SHAs.
...
readonly SETUP_DIR="${HOME}/.config/ubuntu-setup"
...
preflight() {
  ...
  case "${VERSION_ID}" in
    22.04|24.04) ;;
    *) die "This script targets Ubuntu 22.04 / 24.04." ;;
  esac
  ...
  # This script installs bira and other bundled OMZ themes.
  ...
  section "Ubuntu setup started — $(date -Is)"
}
...
setup_system_packages() {
  ...
  if [[ "${INSTALL_FASTFETCH}" == "true" ]]; then
    if apt-cache show fastfetch >/dev/null 2>&1; then
      ...
      warn "Fastfetch is unavailable in configured repositories; skipping."
    fi
  fi
...
install_docker_packages() {
  ...
  # Do not silently remove or replace another Docker distribution.
  local package
  ...
  download "https://download.docker.com/linux/ubuntu/gpg" \
    "${WORK_DIR}/docker.asc"
...
setup_docker() {
  ...
  if [[ "${DOCKER_MODE}" == "rootless" ]]; then
    ...
    # Never stop a potentially active rootful daemon automatically.
...
setup_snaps() {
  ...
  if ! /snap/bin/tldr --update; then
    warn "tldr cache update failed; retry it after setup."
  fi
}
...
setup_zsh() {
  ...
  # Clone directly instead of executing OMZ's moving installer.
  clone_and_pin \
...
setup_aliases() {
  ...
  # Syntax validation is not a security audit.
...
configure_zsh() {
  ...
  local marker="# Managed by setup_ubuntu.sh"
...
source "$HOME/.config/ubuntu-setup/aliases.zsh"
...
EOF
  ...
  # Explicit migration: preserve the old file but do not source it, because
  # doing so could initialize OMZ twice or load previously appended aliases.
  if [[ -e "${ZSHRC_FILE}" ]] &&
...
# Managed by setup_ubuntu.sh
source "$HOME/.config/ubuntu-setup/shell.zsh"
EOF

  install -m 0600 "${WORK_DIR}/zshrc" "${ZSHRC_FILE}.new"
  mv -f -- "${ZSHRC_FILE}.new" "${ZSHRC_FILE}"

  # Deliberately leave .zshenv unchanged.
