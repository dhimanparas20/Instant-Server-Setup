...
sudo apt upgrade -yq
sudo apt install -yq curl uidmap tmate  ufw dnsutils fastfetch net-tools htop network-manager "linux-headers-$(uname -r)"
...
# Reboot prompt
echo -ne "\nDo you want to reboot now(RECOMMENDED)? (y/yes to reboot): "
read answer
